...
## MajorDex implementation overrides (2026-09-06)

The user's established product identity takes precedence over the inspiration below.
Use neutral charcoal (#1C1C1E), white ink, subdued gray, and a clear blue accent.
The first-party constellation-rabbit artwork carries atmosphere; no orange dark theme,
purple gradient, new logo, remote font, tracker, or third-party image is introduced.
Keep generous editorial spacing and clear alignment. System-installed typefaces are
allowed to preserve native Korean/CJK rendering; do not redistribute Apple's fonts.
Build a dependency-free, responsive public landing page plus privacy and support pages,
with all eight app languages available on every page. Use plain external links and no
cookies, localStorage, analytics, signup, fake App Store URL, or unsupported release claims.
Privacy/support prose must remain readable without JavaScript. No app source is published.
